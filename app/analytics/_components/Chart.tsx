...
const Chart = () => {
  ...
  const nutritionTypes = Array.from(analyticsCtx.nutritionTypes).filter((x) =>
    Object.keys(servingValuesUnits).includes(x)
  );
  ...
  return (
    ...
    >
      ...
      {!analyticsCtx.dataIsFetching &&
        Object.keys(analyticsCtx.data).length > 0 &&
        nutritionTypes.length > 0 && (
          <ScrollArea type='always' className='whitespace-nowrap overflow-x-auto'>
            <ChartContainer
              className="min-h-[250px] max-h-full min-w-[600px] max-w-full w-full"
              config={chartConfig}
            >
              <AreaChart
                accessibilityLayer
